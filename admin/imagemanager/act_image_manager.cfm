...
<cfscript>
	...
	if (StructKeyExists( attributes, 'CKEditor') )
		actionURL = actionURL & "&CKEditor=" & attributes.CKEditor;
		
	if (StructKeyExists( attributes, 'CKEditorFuncNum') )
		actionURL = actionURL & "&CKEditorFuncNum=" & attributes.CKEditorFuncNum;
...
<!---						<cfscript>
	...
	<cfif attributes.fieldname is "fckeditor">
		fullimage = attributes.topdir + image ;
		window.opener.SetUrl( fullimage ) ;
	<cfelseif StructKeyExists(attributes, "CKEditor")>	
		fullimage = '#attributes.topdir#' + image ;
		window.opener.CKEDITOR.tools.callFunction(#attributes.CKEditorFuncNum#, fullimage);
