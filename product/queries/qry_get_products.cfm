...
<cfif not (isdefined("attributes.product_searchsubmit") and not Len(trim(searchheader)))>
	...
	<cfif isNumeric(attributes.category_id) AND NOT (attributes.hot OR attributes.onsale OR attributes.notsold OR attributes.new)>
...
<cfset attributes.displaycount = Val(attributes.displaycount)>

<cfset StartRow = (Val(attributes.currentpage) - 1) * attributes.displaycount + 1>
